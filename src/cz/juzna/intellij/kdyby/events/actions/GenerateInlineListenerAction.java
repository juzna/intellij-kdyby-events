...
import cz.juzna.intellij.kdyby.events.PhpIndexUtil;
import org.jetbrains.annotations.NotNull;

/**
 * Created by matej21 on 20.2.15.
 */
public class GenerateInlineListenerAction extends CodeInsightAction {
	...
	private static class GenerateInlineListenerHandler implements LanguageCodeInsightActionHandler {
		...
		public void invoke(@NotNull final Project project, @NotNull Editor editor, @NotNull final PsiFile psiFile) {
			...
			String method = GeneratorUtil.createListenerTemplate(field, el, null);
		...
		private static Field getField(PsiElement el) {
			...
			if (ref.getName().startsWith("on")) {
				for (PhpClass clazz : PhpIndexUtil.getClasses(ref.getClassReference(), el.getProject())) {
					return clazz.findFieldByName(ref.getName(), false);
				}
			}
			return null;
		}
		...
		private static AssignmentExpression getAssignment(PsiElement el) {
			...
			return null;
/*
			if (!(el.getPrevSibling() instanceof Statement) || !(el.getPrevSibling().getFirstChild() instanceof AssignmentExpression)) {
				return null;
			}
*/
		}
